...
with DAG("Mars_DWH_Capstone", default_args=default_args, 
    ...
    download_REMS_to_s3 = PythonOperator(task_id="download_REMS_to_S3", python_callable=download_REMS_s3.main)
    
    create_tables = PostgresOperator(task_id="Initialize_tables",
    ...
    run_quality_checks = DataQualityOperator(
        ...
        dq_checks=[
        {'check_sql': "SELECT COUNT(*) FROM measures WHERE measure_id is null", 'expected_result': 0},
        {'check_sql': "SELECT COUNT(*) FROM time WHERE  time_id is null", 'expected_result': 0},
        {'check_sql': "SELECT COUNT(*) FROM location WHERE  location_id is null", 'expected_result': 0}]
        )
